...
Excitons are bound pairs of electrons populating excited states, and holes (unpopulated electronic states), held together by coloumb attraction. These can be considered a quasi-particle, or an electronic excitation.
In solids, they are formed when a negatively charged electron is excited to a certain state above the fermi level, and also pulled by the positively charged hole, either by the hole it left behind when it was excited, or by another hole, and they both form a single excited state.
If an electron is first excited, and then pulled by a hole to form an exciton, it loses a certain amount of energy to the coloumb attraction, called the binding energy.
This also means, that to uncouple the electron and the hole, an amount of energy, equal to the binding energy is required.\\

\begin{figure}[H]
    \centering
    \begin{minipage}{0.32\linewidth}
    \centering
    \begin{tikzpicture}[baseline, scale=0.75, every node/.style={scale=0.75}]
        % axes
        \draw[black, -Stealth] (-2.5,0) -- (2.5,0) node[right] {$k$};
        \draw[black, -Stealth] (0,-2) -- (0,2.5) node[above] {$E$};
        % bands
        \draw[line width=1.4pt, blue!30!white, domain=-2:2, samples=50, smooth] plot (\x, {0.3*\x*\x+1.5});
        \draw[line width=1.4pt, blue!30!white, domain=-2:2, samples=50, smooth] plot (\x, {-0.3*\x*\x-0.5});
        \node[blue!30!white] at (1.9,1.5) {conduction band};
        \node[blue!30!white] at (1.6,-1.9) {valence band};
        % electron and hole
        \filldraw[blue] (0.4,1.548) circle (2.5pt);
        \node[blue] at (0.9,1.9) {$e^-$};
        \filldraw[red] (-0.4,-0.548) circle (2.5pt);
        \node[red] at (-0.9,-0.5) {$h^+$};
    \end{tikzpicture}
    \end{minipage}
    \hfill
    \begin{minipage}{0.32\linewidth}
    ...
    \begin{tikzpicture}[baseline, scale=0.75, every node/.style={scale=0.75}]
        ...
        \filldraw[red] (-0.4,-0.548) circle (2.5pt);
        % coulomb attraction
        \draw[purple, <->, decorate, decoration={snake, amplitude=1.2pt, segment length=5pt}, line width=1pt]
            (0.4,1.548) -- (-0.4,-0.548);
        \node[purple] at (0.55,0.5) {$F_c$};
    \end{tikzpicture}
    \end{minipage}
    \hfill
    \begin{minipage}{0.32\linewidth}
    ...
    \begin{tikzpicture}[baseline, scale=0.75, every node/.style={scale=0.75}]
        ...
        \draw[line width=1.4pt, blue!30!white, domain=-2:2, samples=50, smooth] plot (\x, {-0.3*\x*\x-0.5});
        \filldraw[red] (-0.4,-0.548) circle (2.5pt);
        % ghost of original electron energy
        \draw[blue, dashed] (0.4,1.548) circle (2.5pt);
        % lowered electron
        \filldraw[blue] (0.4,1.148) circle (2.5pt);
        % ruler marking E_bin
        \draw[black] (0.9,1.548) -- (1.1,1.548);
        \draw[black] (0.9,1.148) -- (1.1,1.148);
        \draw[black] (1.0,1.548) -- (1.0,1.148);
        \node[black, right] at (1.15,1.348) {$E_{bin}$};
    \end{tikzpicture}
    \end{minipage}
    \caption{Effect of the Coulomb attraction on an exciton's electron energy. (a) An electron in the conduction band and a hole in the valence band, with no interaction shown. (b) The Coulomb attraction $F_c$ between the electron and the hole. (c) As a bound pair, the electron's energy is reduced by the binding energy $E_{bin}$ relative to a free conduction-band electron at the same momentum.}
    \label{fig:exciton_binding_diagram}
\end{figure}

An electron-hole pair composing an exciton in a solid crystal can be described as a pair of two particles with opposing charges, pulling on each other, while being screened by the lattice, those two form a hydrogen like series of states, that can be thought of as a picture of the electron and the hole circling eachother at a certain radius.\\
Excitons in solid crystals are commonly divided to two kinds:
\begin{itemize}
    \item Frenkel excitons~\cite{frenkel1931transformation} - with roughly the size of a single unit cell, and binding energies in the order of magnitude of $0.1eV$.
    \item Wannier excitons~\cite{wannier1937structure} - loosely bound, with characteristic sizes spanning multiple unit cells, and binding energies in the order of magnitude of $0.01eV$.
\end{itemize}

Given that the binding energy of the electron-hole pair is that of a pair of charged particles, there should be a Rydberg series of possible binding energies, depending on the angular momentum of the pair.\note{citation?}\\
\note{the key features observable through ARPES are the modified energy, and the unusual lifetime. modified energy is explained by the binding energy, and the unusual lifetime requires more proof - TODO - find theoretical framework for extended lifetime}\\
\note{Ideally I would now list theoretical features of excitons and then in the next section about measurements, list experiments that measured the same features. but I don't think I have enough such articles in my hand}\\
\note{clear notes to  myself on excitons in published research}\\
...
5. spectral lines + Rydberg series in the reflectivity spectrum.
\\
\note{clear notes to  myself on excitons in published research}

\subsubsection{Exciton Measurements}
Originally, the existence of excitons was demonstrated by measuring the absorption spectrum of a copper oxide~\cite{gross1952optical}, and demonstrating that beside the absorption peaks at band transitions, there were multiple peaks at series of energies near band transitions.
Closer examination of those energies showed that they followed the familiar Rydberg series pattern as theorized, serving as vivid proof that bound particles served a role in determining the interaction of the oxide with light.
Other kinds of light-matter interactions also turn out behavior similar to Rydberg series, that is very suggestive of the presence of excitons: photoluminescence \note{citation?}, reflectivity~\cite{chernikov2014exciton}, etc...\\


When it comes to ARPES measurements demonstrating behavior of excitons, there is less abundance.
The first excitonic feature that would be visible in an ARPES measurement, is the modified state energy.
As demonstrated in \hyperref[fig:exciton_binding_diagram]{Figure~\ref{fig:exciton_binding_diagram}}, an electron photo-emitted from an excitonic state, would appear at an energy slightly lower than an existing electron band~\cite{man2021direct}.
In theory, if there are multiple Rydberg states with different energy levels, an ARPES measurement could demonstrate it, however there are limitations to the energy
resolution, and also to the sensitivity to states with small populations.
In practice there is no evidence for Rydberg series energy levels in ARPES measurements.
The second excitonic feature that can be measured in time resolved ARPES is an extended lifetime~\cite{bi2te3_excitons_lifetime}.
\\
\note{clear notes to myself on excitons measurements in published research}\\
...
\note{clear notes to myself on excitons measurements in published research}
\subsection{$Bi_2Se_3$}
\note{add here plot of DFT with Bi2Se3 band structure}\\
\note{add here measured band structure of Bi2Se3 below fermi level from Ittai's article}\\
\subsection{Photoluminescence in $Bi2Se3$}
\note{check crystal growth in PL paper, and look specifically for doping}\\
